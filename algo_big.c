	...
	while (tmp)
	{
		while (tmp->number)
		{
			...
			if (tmp->stay == 0)
			{
				...
				tmp->size--;
				ft_index(tmp);
				printf("Sorter 1 : tmp->number = %d, tmp->stay = %d\n\n", tmp->number, tmp->stay);
				break ;
			}
			else if (tmp->stay == 1)
			{
				ft_rotate(&tmp, 'a');
				ft_index(tmp);
				printf("Sorter 2 : tmp->number = %d, tmp->stay = %d\n", tmp->number, tmp->stay);
				break ;
			}
		}
